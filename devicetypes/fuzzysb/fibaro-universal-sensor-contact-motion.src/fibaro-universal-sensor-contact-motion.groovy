	...
	if (cmd.commandClass == 32) {
		if (cmd.parameter == [0]) {
			if (cmd.sourceEndPoint == 1) {
				return createEvent(name: "contact", value: "closed", descriptionText: "$device.displayName is closed", isStateChange: true)
				log.debug "Contact is closed"
			}
			else
			if (cmd.sourceEndPoint == 2) {
				return createEvent(name: "motion", value: "active", descriptionText: "$device.displayName is active", isStateChange: true)
				log.debug "motion is active"
			}
		}
		if (cmd.parameter == [255]) {
			if (cmd.sourceEndPoint == 1) {
				return createEvent(name: "contact", value: "open", descriptionText: "$device.displayName is open", isStateChange: true)
				log.debug "Contact is open"
			}
			else
			if (cmd.sourceEndPoint == 2) {
				return createEvent(name: "motion", value: "inactive", descriptionText: "$device.displayName is inactive", isStateChange: true)
