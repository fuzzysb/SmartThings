 ...
 *  Date: 2016-02-01 v1.2 amended temperature selection for Farenheit
	...
	tiles(scale: 2){
      	multiAttributeTile(name: "thermostat", type:"thermostat", width:6, height:4) {
			tileAttribute("device.temperature", key:"PRIMARY_CONTROL", canChangeIcon: true, canChangeBackground: true){
            	attributeState "default", label:'${currentValue}° F', unit:'F', backgroundColor:"#fab907", icon:"st.Home.home1"
            }
...
private parseweatherResponse(resp) {
    ...
    if(resp.status == 200) {
        ...
        def outsidetemperature = Math.round(resp.data.outsideTemperature."${temperatureUnitName}")
...
def setCoolingTempCommand(targetTemperature){	
    ...
    def jsonbody = new groovy.json.JsonOutput().toJson([setting:[fanSpeed:"HIGH", mode:"COOL", power:"ON", temperature:["${temperatureUnitName}":targetTemperature], type:"AIR_CONDITIONING"], termination:[type:"TADO_MODE"]])
	sendCommand("temperature",[jsonbody])
}
...
def setHeatingTempCommand(targetTemperature){
    ...
    def jsonbody = new groovy.json.JsonOutput().toJson([setting:[fanSpeed:"AUTO", mode:"HEAT", power:"ON", temperature:["${temperatureUnitName}":targetTemperature], type:"AIR_CONDITIONING"], termination:[type:"TADO_MODE"]])
	sendCommand("temperature",[jsonbody])
}
...
def onCommand(){
    ...
    def jsonbody = new groovy.json.JsonOutput().toJson([setting:[fanSpeed:"AUTO", mode:"COOL", power:"ON", temperature:["${temperatureUnitName}":"${onvalue}"], type:"AIR_CONDITIONING"], termination:[type:"TADO_MODE"]])
	sendCommand("temperature",[jsonbody])
}
