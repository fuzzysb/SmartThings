 ...
 *	Date: 2016-05-07 v2.9 Huge Rewrite to optimise code alse added Extra checks regarding AC capability. also configured default values and extra checks if the defaults return null. 
						  and also added Device Handler tile sets for easy editing based on capability. many thanks to @infofiend & @RudiP for thier invaluable input. 
						  but please do expect bugs with this release as it is a big rewrite to optimise the previously unweildy code.
...
metadata {
	...
	tiles(scale: 2){
		...
		main(["thermostat"])
		//Default TileSet, all Cooling/Heating/Auto/Dry/Fan with Fan Speed Tiles enabled.
		...
		//All Modes without Fan Speeds, all Cooling/Heating/Auto/Dry/Fan with Fan Speed Tiles enabled.
		//details(["thermostat","thermostatMode","coolingSetpointUp","coolingSetpointDown","autoOperation","heatingSetpointUp","heatingSetpointDown","outsidetemperature","thermostatSetpoint","tadoMode","refresh","setAuto","setOn","setOff","fan","cool","heat","setDry","emergencyHeat","endManualControl"])
		
		//Cooling/Auto/Dry/Fan with Fan Speed TileSet, No Heating comment out default and enable this one.
		//details(["thermostat","thermostatMode","coolingSetpointUp","coolingSetpointDown","autoOperation","outsidetemperature","thermostatSetpoint","tadoMode","refresh","tadoFanSpeed","setAuto","setOn","setOff","fan","cool","setDry","cmdFanSpeedAuto","emergencyHeat","endManualControl","cmdFanSpeedLow","cmdFanSpeedMid","cmdFanSpeedHigh"])
		
		//Cooling/Auto/Dry/Fan without Fan Speed TileSet, No Heating comment out default and enable this one.
		//details(["thermostat","thermostatMode","coolingSetpointUp","coolingSetpointDown","autoOperation","outsidetemperature","thermostatSetpoint","tadoMode","refresh","setAuto","setOn","setOff","fan","cool","setDry","emergencyHeat","endManualControl"])
		
		//Cooling only with Fan Speed TileSet, No Heating/Auto/Dry/Fan comment out default and enable this one.
		//details(["thermostat","thermostatMode","coolingSetpointUp","coolingSetpointDown","autoOperation","outsidetemperature","thermostatSetpoint","tadoMode","refresh","tadoFanSpeed","setOn","setOff","cool","cmdFanSpeedAuto","emergencyHeat","endManualControl","cmdFanSpeedLow","cmdFanSpeedMid","cmdFanSpeedHigh"])
		
		//Cooling only without Fan Speed TileSet, No Heating/Auto/Dry/Fan comment out default and enable this one.
		//details(["thermostat","thermostatMode","coolingSetpointUp","coolingSetpointDown","autoOperation","outsidetemperature","thermostatSetpoint","tadoMode","refresh","setOn","setOff","cool","emergencyHeat","endManualControl"])
	}
}
...
private parseCapabilitiesResponse(resp) {
    ...
    if(resp.status == 200) {
        ...
        if(resp.data.COOL || (resp.data.COOL).toString() == "[:]"){
			...
			try{
				coolfanmodelist = resp.data.COOL.fanSpeeds
			}catch(Exception e){
				log.debug("No Cooling Fan Speed Capability")
				coolfanmodelist = "NONE"
			}
            if (coolfanmodelist != "NONE") {
				log.debug("setting state.SupportsCoolFanSpeedSetting to True")
				state.SupportsCoolFanSpeedSetting = "true"
				...
				log.debug("setting state.SupportsCoolFanSpeedSetting to False")
				state.SupportsCoolFanSpeedSetting = "false"
			}
            if (state.tempunit == "C"){
        ...
        if(resp.data.HEAT || (resp.data.HEAT).toString() == "[:]"){
			...
			try{
				heatfanmodelist = resp.data.HEAT.fanSpeeds
			}catch(Exception e){
				log.debug("No Heating Fan Speed Capability")
				heatfanmodelist = "NONE"
			}
            if (heatfanmodelist != "NONE") {
				log.debug("setting state.SupportsHeatFanSpeedSetting to True")
				state.SupportsHeatFanSpeedSetting = "true"
				...
				log.debug("setting state.SupportsHeatFanSpeedSetting to False")
				state.SupportsHeatFanSpeedSetting = "false"
			}
            if (state.tempunit == "C"){
		...
		log.debug("state.SupportsCoolFanSpeedSetting = ${state.SupportsCoolFanSpeedSetting}")
		...
		log.debug("state.SupportsHeatFanSpeedSetting = ${state.SupportsHeatFanSpeedSetting}")
    }catch(Exception e){
...
def refresh() {
    if (!(state.SupportsCoolFanSpeedSetting || state.tempunit )){
		updated()
	}
...
def autoCommand(){
    ...
    if (capabilitysupported == "true"){
		modeCommandBuilder("AUTO","ON",null,null)
    } else {
...
def dryCommand(){
    ...
    if (capabilitysupported == "true"){
		modeCommandBuilder("DRY","ON",null,null)
    } else {
...
def fanAuto(){
    ...
    if (capabilitysupported == "true"){
		modeCommandBuilder("FAN","ON",null,null)
	} else {
...
def cmdFanSpeedAuto(){
	...
	if (curMode == "COOL" || curMode == "HEAT") {
		def supportedfanspeed = "AUTO"
		def curSetTemp = getCurrentSetPointTemp()
		modeCommandBuilder(curMode,"ON",targetTemperature,supportedfanspeed)
	}
}
...
def setCoolingTempCommand(targetTemperature){
	modeCommandBuilder("COOL","ON",targetTemperature,null)
}

def setHeatingTempCommand(targetTemperature){
	modeCommandBuilder("HEAT","ON",targetTemperature,null)
}

def offCommand(){
	log.debug "Executing 'sendCommand.offCommand'"
	modeCommandBuilder(null,"OFF",null,null)
}

def getCurrentSetPointTemp(){
	def currentSetPointTemp
    try {
        currentSetPointTemp = ((device.currentValue("thermostatSetpoint")).intValue())
    }catch (NumberFormatException e){
         currentSetPointTemp = 0 
    }
	return currentSetPointTemp
}

def getSupportedFanSpeed(mode,fanSpeedSetting){
    def supportedfanspeed
    if (mode == "COOL" && fanSpeedSetting != null) {
	switch (fanSpeedSetting) {
    	case "AUTO":
        	if (state.SupportsCoolAutoFanSpeed == "true"){
				supportedfanspeed = "AUTO"
			} else if (state.SupportsCoolAutoFanSpeed == "false") {
				supportedfanspeed = "HIGH"
			}
        break
    	case "HIGH":
        	supportedfanspeed = fanSpeedSetting
        break
    	case "MIDDLE":
        	supportedfanspeed = fanSpeedSetting
        break
	...
	} else if (mode == "HEAT" && fanSpeedSetting != null) {
	switch (fanSpeedSetting) {
    	case "AUTO":
        	if (state.SupportsCoolAutoFanSpeed == "true"){
				supportedfanspeed = "AUTO"
	...
	} else if ((mode == "COOL" || mode == "HEAT") && fanSpeedSetting == null) {
		if (state.SupportsCoolAutoFanSpeed == "false" || state.SupportsCoolAutoFanSpeed == "false") {
			supportedfanspeed = "HIGH"
		} else {
			supportedfanspeed = "AUTO"
		}
	} else {
		supportedfanspeed = null
	}
	return supportedfanspeed
}


def modeCommandBuilder(mode,power,targetTemperature,fanSpeedSetting){
	log.debug("entered modeCommandBuilder")
	def terminationmode = settings.manualmode ?: "TADOMODE"
    def supportedfanspeed = null
	def fanCommand
	def powerCommand
	def tempCommand
	def curMode
	...
	def modeCommandJson
	def fullCommand
	def currentSetPointTemp = getCurrentSetPointTemp()
	if (power == "ON") {
    	log.debug("Power Setting is ${power}")
	    powerCommand = /power:"${power}", /
		curMode = /mode:"${mode}", /
	} else if (power == "OFF"){
    	log.debug("Power Setting is ${power}")
		curMode = ""
	...
	if (state.SupportsCoolFanSpeedSetting == "true" || state.SupportsHeatFanSpeedSetting == "true"){
    	log.debug("getting supported fan Speed")
		supportedfanspeed = getSupportedFanSpeed(mode,fanSpeedSetting)
        log.debug("supportedfanSpeed = ${supportedfanspeed}")
	}
	if (mode == "COOL") {
		if (supportedfanspeed != null) {
			fanCommand = /fanSpeed:"${supportedfanspeed}", /
		} else {
			fanCommand = ""
		}
			if (state.tempunit == "C"){
			initialsetpointtemp = settings.defCoolingTemp ?: 21
			tempUnitName = "celsius"
		} else if (state.tempunit == "F") {
			initialsetpointtemp = settings.defCoolingTemp ?: 70
			tempUnitName = "fahrenheit"
		}
		if ( targetTemperature == null && mode != null ) {
			tempCommand = /temperature:[tempUnitName:initialsetpointtemp], /
        } else if (targetTemperature == null && mode != null && targetTemperature == null && currentSetPointTemp != 0){
			tempCommand = /temperature:[tempUnitName:currentSetPointTemp], /
		} else {
			tempCommand = /temperature:[tempUnitName:targetTemperature], /
		}
		...
		if (state.tempunit == "C"){
			initialsetpointtemp = settings.defHeatingTemp ?: 21
			tempUnitName = "celsius"
		} else if (state.tempunit == "F") {
			initialsetpointtemp = settings.defHeatingTemp ?: 70
			tempUnitName = "fahrenheit"
		}
		if ( targetTemperature == null && mode != null && targetTemperature == null && currentSetPointTemp == 0) {
			tempCommand = /temperature:[tempUnitName:"initialsetpointtemp], /
		} else if (targetTemperature == null && mode != null && targetTemperature == null && currentSetPointTemp != 0){
			tempCommand = /temperature:[tempUnitName:currentSetPointTemp], /
		} else {
    ...
    //log.debug("${fanCommand}")
    log.debug("${curMode}")
	//fullCommand = (/[setting:[${fanCommand}${curMode}${powerCommand}${tempCommand}type:"AIR_CONDITIONING"], termination:[type:"${terminationmode}"]]/)
    log.debug("Executing 'sendCommand' with the following Body" + fullCommand)
    def jsonbody =  new groovy.json.JsonOutput().toJson([setting:[fanCommand curMode powerCommand tempCommand type:"AIR_CONDITIONING"], termination:[type:terminationmode]])
    log.debug(jsonbody)
    //jsonSlurper = jsonSlurper.ParseText(jsonbody)
	log.debug("Executing 'sendCommand' with the following Body" + fullCommand)
...
def onCommand(){
	modeCommandBuilder("COOL","ON",null,null)
}
...
def heatCommand(){
	modeCommandBuilder("HEAT","ON",null,null)
}
